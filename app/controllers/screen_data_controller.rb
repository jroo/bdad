class ScreenDataController < ApplicationController
  ...
  def create
    ...
    unless params[:district_code] && params[:d] && params[:offset]
      render :text => "Need district_code, d, and offset", :status => 401
      return
    end

    screen_data.offset_x = params[:offset][:x]
    screen_data.offset_y = params[:offset][:y]
